...
import { ClrVerticalNavGroup, ClrVerticalNavGroupChildren } from '@clr/angular';
...
export class DashboardsComponent implements OnInit, OnDestroy {
  ...
  getTooltipTitle(event: DashboardScheduledEvent): string {
    ...
    if (event.activeSessions > 0) {
      tooltipTitle = `${event.activeSessions} active sessions`;
    }
    if(event.provisionedVMs && event.provisionedVMs > 0) {
      tooltipTitle += ` and ${event.provisionedVMs} provisioned vms`
    }
    if(!tooltipTitle) {
