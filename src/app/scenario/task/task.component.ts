...
export class TaskComponent {
  ...
  @Input() set selectedScenario(scenario: Scenario) {
    ...
    this.vmNames = scenario.virtualmachines.flatMap(vmSet => Object.keys(vmSet))
    
    this.vmTasks.forEach(vmTask => {
      vmTask.tasks.forEach((task: Task) => {
        let editTask = {vmName: vmTask.vm_name, id: this.getRandomId(), ...task}
  ...
  buildEditedVMTasks() {
      ...
      this.changedTasks.forEach(editTask => {
        let vmTask = updatedVMTasks.find(vmTask => {return vmTask.vm_name == editTask.vmName})
        if (vmTask) {
          vmTask.tasks.push(this.buildTask(editTask))
        }
        else {
          updatedVMTasks.push({vm_name: editTask.vmName, tasks: [this.buildTask(editTask)]})
        }
      })
  ...
  updateTask(changedTask: EditTask) {
    let idExists = this.changedTasks.map(task => task.id).includes(changedTask.id)
    if (!idExists) {
      this.changedTasks.push(changedTask)
      return
    }
    this.changedTasks = this.changedTasks.map(task => task.id !== changedTask.id ? task : changedTask)
    this.buildEditedVMTasks()
  }

  addTask() {
    let newTask = {vmName: this.vmNames[0] ?? '', id: this.getRandomId(), ...this.buildTask()} as EditTask
  ...
  deleteTask(id: string) {
    this.editTasks = this.editTasks.filter((task: EditTask) => {return task.id !== id})
    this.changedTasks = this.changedTasks.filter((task: EditTask) => {return task.id !== id})
    this.buildEditedVMTasks()
  }

  applyChanges() {
    this.changedTasks.forEach((chTask: EditTask) => {
      const alreadyExists = this.editTasks.map(task => task.id).includes(chTask.id)
      if (alreadyExists) {
        this.editTasks = this.editTasks.map((eTask: EditTask) => {return eTask.id == chTask.id ? chTask : eTask})
      } else {
